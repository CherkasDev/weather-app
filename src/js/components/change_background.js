...
function getLinkToImage(city, zone) {
  ...
  const url = `https://api.unsplash.com/photos/random?query=${addBgParameters(city, zone)},city, weather&client_id=${id}`;
  console.log(`key words for background are ${city}, ${zone}, weather`);
  return fetch(url)
    .then((res) => res.json());
}
